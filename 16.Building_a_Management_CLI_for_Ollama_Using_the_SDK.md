...
```text
SDK version:    0.6.2
Server host:    http://localhost:11434
Models on disk: 3
```
...
# entry-point table: maps commands to functions
[project.scripts]                    
mycli = "mycli:main"                 

[build-system]                       
# build dependency: the hatchling backend
requires = ["hatchling"]             
build-backend = "hatchling.build"    

[tool.hatch.build.targets.wheel]     
include = ["mycli.py", "config.py"]   
```
...
# Show a model
uv run mycli show qwen2.5:3b

# Load a model (server side)
